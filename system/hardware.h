...
inline bool Hardware::gpu_query()
{
	printf(" CUDA Device Query (Runtime API) \n\n");
	int count = 0;
	cudaError_t eid = cudaGetDeviceCount(&count);
	if (eid != cudaSuccess)
	{
		printf("cudaGetDeviceCount returned %d\n-> %s\n", (int)eid, cudaGetErrorString(eid));
		return false;
	}
	if (count == 0) printf("There are no available device(s) that support CUDA\n");
	else printf("Detected %d CUDA Capable device(s)\n", count);
	// from the cuda sample code
