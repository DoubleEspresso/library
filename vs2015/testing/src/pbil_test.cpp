...
float residual_function2(int * sample, void * params)
{
	...
	for (int r = 0, idx = 0; r < p->rows; ++r)
	{
		for (int c = 0; c < p->cols; ++c, idx += p->bits_per_int)
		{
			...
			//printf("elmt(%d,%d)=%d\n", r, c, integer(sample, idx, idx + p->bits_per_int)); 
		}
	}
	...
	//for (int j = 0; j < p->total_length; ++j) printf("%d", sample[j]);
	//printf("\n");
	//for (int r = 0; r < p->rows; ++r)  printf("rowsum(r=%d)=%d\n", r, rowsum[r]);
	//for (int c = 0; c < p->cols; ++c)  printf("colsum(c=%d)=%d\n", c, colsum[c]);
